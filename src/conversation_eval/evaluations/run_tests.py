...
def parse_arguments():
    ...
    Format: run_tests.py <test_suite> [npc_type1 npc_type2 ...] [test_name]
    - First arg: test suite folder name (required, e.g., memory_wipe_tests)
    ...
    Returns:
        tuple: (test_suite: str, npc_types: List[str], specific_test: Optional[str])
    """
    valid_npc_types = {"npc0", "npc1", "npc2"}
    
    # First argument must be test suite name
    ...
    return test_suite, npc_types, specific_test
...
def main():
    ...
    # Parse arguments
    test_suite, npc_types, specific_test = parse_arguments()
    
    if test_suite is None or (len(sys.argv) > 1 and sys.argv[1] in ["--help", "-h"]):
        print("Usage: python run_tests.py <test_suite> [npc_type1 npc_type2 ...] [test_name]")
        print("  test_suite: name of test suite folder (e.g., memory_wipe_tests)")
        ...
        print("\nExamples:")
        print("  python run_tests.py memory_wipe_tests                                 # Run all tests for all NPCs")
        print("  python run_tests.py memory_wipe_tests npc0                            # Run all tests for npc0")
        print("  python run_tests.py memory_wipe_tests npc0 npc1                       # Run all tests for npc0 and npc1")
        print("  python run_tests.py memory_wipe_tests emotional_escalation_response   # Run specific test for all NPCs")
        print("  python run_tests.py memory_wipe_tests npc0 emotional_escalation_response  # Run specific test for npc0")
        sys.exit(0)
    
    # Get directories - eval_dir is now the test suite folder
    eval_dir = Path(__file__).parent / test_suite
    
    if not eval_dir.exists():
        print(f"Error: test suite directory not found at {eval_dir}")
        sys.exit(1)
