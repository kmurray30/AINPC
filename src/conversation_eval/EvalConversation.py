...
class EvalConversation:
    ...
    agents: Dict[AgentName, EvalConvoMember]

    def __init__(self):
        self.message_history = []
        self.agents = {}

    def add_agent_simple(self, name: AgentName, agent_rules: List[str]):
        agent = EvalConvoMember(name, "\n".join(agent_rules))
        self.agents[name] = agent

    def add_agent_with_npc_protocol(self, name: AgentName, agent_rules: List[str], npc_protocol: NPCProtocol) -> EvalConvoMember:
    ...
    # For each conversing instance, one of the agents acts as the 'assistant' since it is the one that is being put in the assistant role and called as the LLM
    # The other agent acts as the 'user' since it is the that the LLM sees as the other
    def call_agent(self, assistant_agent_name: AgentName, user_agent_name: AgentName, response_is_typed: bool, isPrinting = False):
        # Check if the agents have been added
        if assistant_agent_name not in self.agents:
            raise ValueError(f"Agent {assistant_agent_name} not found in agents list.")

        assistant_agent = self.agents[assistant_agent_name]
        
        # All agents are now NPC-backed
        # Get the last message from the user agent as user input
        last_message = None
        if self.message_history:
            # Find the most recent message from the user agent
            for msg in reversed(self.message_history):
                if msg.agent == user_agent_name:
