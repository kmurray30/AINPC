"""
Terminal UI Manager for Parallel Evaluation Execution - V2
Uses clear-and-redraw approach instead of cursor save/restore for better compatibility
"""
import threading
...
class TerminalUI_v2:
    ...
    def _render(self):
        """Render the entire UI from scratch"""
        with self.lock:
            # Clear screen and move to top
            print(CLEAR_SCREEN + MOVE_TO_TOP, end='')
            
            # Print header
            print(f"\n{'='*60}")
            print(f"{BOLD}🧪 Test: {self.test_name} ({self.npc_type.upper()}){RESET}")
            print(f"{'='*60}\n")
            
            # Print each case
            for case_idx, eval_case in enumerate(self.eval_cases, 1):
                print(f"{'─'*60}")
                print(f"{BOLD}📝 Case {case_idx}/{len(self.eval_cases)}{RESET}")
                ...
                for convo_idx in range(1, self.convos_per_user_prompt + 1):
                    ...
                    print(f"Conversation {convo_idx}:")
                    print(f"    {convo_state.generating_progress}")
                    print(f"    {convo_state.evaluation_progress}\n")
