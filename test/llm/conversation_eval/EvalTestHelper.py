...
def calculate_accuracy(conversation_actual_outcome: EvaluationResponseEvalTestReport, conversation_expected_outcome: ConversationOutcome) -> float:
...
# Take in a list of EvaluationEvalTestReports and return a list of EvalTestReports (calculating how accurate the evaluations were)
def _generate_eval_report(eval_test_reports: List[EvaluationEvalReport], conversation_map: Dict[str, List[str]], conversations_expected_results: Dict[str, ConversationOutcome], eval_iterations: int) -> EvalTestReport:
    eval_report = EvalTestReport(evaluations={}, conversations=len(conversation_map), iterations=eval_iterations, timestamp_accuracies={}, result_accuracies={}, timestamp_accuracy=0, result_accuracy=0, tokens=0)
    for evaluation_test_report in eval_test_reports:
        condition_tr = evaluation_test_report.evaluation_proposition
        antecedent_er = TermEvalTestReport(value=condition_tr.antecedent.value, negated=condition_tr.antecedent.negated) if condition_tr.antecedent is not None else None
        consequent_er = TermEvalTestReport(value=condition_tr.consequent.value, negated=condition_tr.consequent.negated)
        ...
        proposition_er = PropositionEvalTestReport(antecedent=antecedent_er, consequent=consequent_er, min_responses_for_consequent=min_responses_conq, max_responses_for_consequent=max_responses_conq, max_responses_for_antecedent=max_responses_ant)
        evaluation_er = EvaluationEvalTestReport(proposition=proposition_er, conversation_evaluations=[], timestamp_accuracy=0, result_accuracy=evaluation_test_report.result_score, tokens=0)
        for conversation_evaluation in evaluation_test_report.conversation_evaluations:
            ...
            conversation_er = ConversationEvaluationEvalTestReport(
            ...
            for evaluation_iteration in evaluation_iterations:
                ...
                evaluation_response = EvaluationResponseEvalTestReport(
                ...
                evaluation_iteration_eval_report = EvaluationIterationEvalTestReport(
...
def write_eval_test_report_to_file(eval_report: EvalTestReport, test_name: str = ""):
    # Write the test report to a json file
    current_time = Utilities.get_current_time_str()
    test_report_path = Utilities.get_path_from_project_root(f"test/llm/conversation_eval/reports/EvalTestReport_{test_name}_{current_time}.json")
...
def generate_eval_report_test_helper(proposition: Proposition, conversations_path: str, conversations_expected_event_times: Dict[str, ConversationOutcome], eval_iterations: int) -> EvalTestReport:
    EvalHelper.validate_input(proposition)
    ...
    # Run the evaluations
    evaluation_test_reports = EvalHelper.run_evaluations_on_conversation(conversation_map, [proposition], eval_iterations)
