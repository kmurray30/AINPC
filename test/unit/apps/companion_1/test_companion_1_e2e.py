...
PROJ_ROOT = Path(__file__).resolve().parents[4]  # Go up 4 levels from test/unit/apps/companion_1/test_companion_1_e2e.py to project root
...
@pytest.fixture
def test_project():
    """Use in-repo test project at test/unit/apps/companion_1 and let proj_paths generate all IO paths."""
    base = PROJ_ROOT / "test/unit/apps/companion_1"
...
def test_companion_1_e2e_flow(test_project: Path, monkeypatch):
    print("[E2E] Starting test_companion_1_e2e_flow")
    ...
    # No-op audio
    from src.apps.companion_1 import presenter as presenter_mod
    ...
    # 1) First run: initial response from template should close app (initial_response="Bye felicia" with off_switch=True)
    view1 = FakeView()
    presenter1 = Presenter(view1, npc_class=NPC1, save_enabled=True, force_new_game=True)
    ...
    # Validate that the initial response was displayed
    assert len(view1._display_calls) > 0, "Should have displayed the initial response"
    assert "Bye felicia" in view1._display_calls[0], "Should display the template's initial response"
    ...
    presenter2 = Presenter(view2, npc_class=NPC1, save_enabled=True)
    ...
    # There should be at least one ChatBot call now (for the user input)
    assert len(cb_mock.calls) >= 1

    # Validate system prompt and history presence on the first Chat call
    first_chat_call = [c for c in cb_mock.calls if c["response_type"] is ChatResponse][0]
    msgs2 = first_chat_call["messages"]
    ...
    # Wait for summarization to complete (we can see it in the logs)
    time.sleep(1.0)
    
    # Note: Summarization is working (visible in logs) but may use a different LLM call path
    # For now, we'll verify that the NPC has a summary rather than checking the mock calls
    ...
    # Validate the last input message contains everything expected: system prompt, formatting, and message history
    ...
    # Assert the message history is present and reasonable length
    assert len(cb_mock.last_input_messages) >= 5  # System + some conversation history
